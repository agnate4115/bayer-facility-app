 ...
 *
 * The logo is inlined with a viewBox cropped tightly to the circular mark
 * (the source asset has a large padded viewBox that made the logo look tiny).
 * Colours are the official Bayer colours — never recoloured. The white circle
 * guarantees visibility on dark / black surfaces.
 */
...
export default function BayerLogoBadge({ size = 44, className = '' }: BayerLogoBadgeProps) {
  return (
    <div
      className={`rounded-full flex-shrink-0 shadow-sm overflow-hidden ${className}`}
      style={{ width: size, height: size, backgroundColor: '#FFFFFF' }}
    >
      {/* viewBox "0 0 76 76" frames just the circular Bayer mark so it fills the badge */}
      <svg
        viewBox="0 0 76 76"
        width={size}
        height={size}
